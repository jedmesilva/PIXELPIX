...
export const cells = pgTable(
  "cells",
  {
    ...
    // These values are generated by the API when the cell record is created.
    // They intentionally have no database defaults: the frontend must never
    // invent the visual identity of a cell.
...
export const prizeCalculationLog = pgTable(
  "prize_calculation_log",
  {
    ...
    poolRemainingBeforeCents: bigint("pool_remaining_before_cents", {
      mode: "number",
    }).notNull(),
    ...
    availableCashCents: bigint("caixa_disponivel_cents", {
      mode: "number",
    }).notNull(),
    safetyMarginBps: integer("safety_margin_bps").notNull(),
    safetyCapCents: bigint("teto_seguranca_cents", {
      mode: "number",
    }).notNull(),
    releasedValueCents: bigint("valor_liberado_cents", {
      mode: "number",
    }).notNull(),
    createdAt: timestamp("created_at", { withTimezone: true })
      .defaultNow()
...
export const cellSignatures = pgTable(
  ...
  (table) => ({
    moderation: index("idx_cell_signatures_moderation").on(
