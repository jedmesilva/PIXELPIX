...
function OverviewContent() {
  ...
  const data = overview.data;
   ...
   if (!accessKey) return <><PageHeader eyebrow="Operação / visão geral" title="Bom dia, equipe." description="Veja o estado financeiro, os sorteios ativos e o que precisa de ação agora." /><AccessKeyPrompt onSaved={(key) => { saveAccessKey(key); setShowKey(false); }} /></>;
   if (overview.isLoading || recent.isLoading) return <><PageHeader eyebrow="Operação / visão geral" title="Bom dia, equipe." description="Veja o estado financeiro, os sorteios ativos e o que precisa de ação agora." /><LoadingPanel rows={5} /></>;
  if ((overview.isError || recent.isError) && (isAccessError(overview.error) || isAccessError(recent.error))) {
     return <><PageHeader eyebrow="Operação / visão geral" title="Bom dia, equipe." description="Veja o estado financeiro, os sorteios ativos e o que precisa de ação agora." />{showKey ? <AccessKeyPrompt onSaved={(key) => { saveAccessKey(key); setShowKey(false); }} /> : <ErrorState accessRequired onRetry={() => setShowKey(true)} />}</>;
  }
   if (overview.isError || !data) return <><PageHeader eyebrow="Operação / visão geral" title="Bom dia, equipe." /><ErrorState onRetry={() => overview.refetch()} /></>;
  ...
  return <div className="space-y-8">
     <PageHeader eyebrow="Operação / visão geral" title="Bom dia, equipe." description="Veja o estado financeiro, os sorteios ativos e o que precisa de ação agora." action={<button className="button button-secondary" onClick={() => { overview.refetch(); recent.refetch(); }} data-testid="button-refresh-overview"><RefreshCw size={15} /> Atualizar</button>} />
    ...
    <div className="grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
      <section className="panel overflow-hidden" data-testid="panel-financials">
         <SectionHeading kicker="A — leitura financeira" title="Dinheiro em movimento" detail="Planejado, liberado e pago aparecem separados para evitar interpretações erradas." />
      ...
      <section className="panel" data-testid="panel-cells">
         <SectionHeading kicker="B — grade" title="Estado das células" detail={`${cellTotal.toLocaleString('pt-BR')} posições do grid`} />
