      ...
      <div className="mt-auto p-4">
        ...
        <div className="mt-4 flex items-center gap-3 border-t border-sidebar-border pt-4">
          <div className="grid size-8 place-items-center rounded-full bg-[#ff8e70] text-xs font-bold text-[#202a2f]">OP</div>
            <div className="min-w-0"><div className="truncate text-xs font-semibold">Mesa de operações</div><div className="text-[11px] text-sidebar-foreground/45">Operador autorizado</div></div>
  ...
  return (
    <div className="min-h-[100dvh] bg-background text-foreground">
      ...
      <div className="lg:pl-[248px]">
        ...
        <main className="control-grid min-h-[calc(100dvh-76px)] px-4 py-6 sm:px-8 sm:py-8 lg:px-10"><div className="admin-content">{children}</div></main>
      </div>
    </div>
  );
}
...
export function PageHeader({ eyebrow, title, description, action }: { eyebrow: string; title: string; description?: string; action?: React.ReactNode }) {
  return <div className="page-header"><div className="min-w-0"><div className="mb-2 flex items-center gap-2 font-mono-ui text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground"><Activity size={13} className="text-[#789a31]" />{eyebrow}</div><h1 className="text-3xl font-bold tracking-[-0.045em] text-foreground sm:text-[38px]">{title}</h1>{description && <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{description}</p>}</div>{action && <div className="shrink-0">{action}</div>}</div>;
}
